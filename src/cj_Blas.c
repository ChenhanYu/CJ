...
//gemm for non-transpose non-transpose
...
//syrk (symetric rank k update), C: lower-triangular + A: non-triangular
void cj_Syrk_ln_blk_var5 (cj_Object *A, cj_Object *C) {
  cj_Object *AL,    *AR,       *A0,  *A1,  *A2;

  fprintf(stderr, "Syrk_nn_blk_var5 (A(%d, %d), C(%d, %d)): \n", 
      A->matrix->m, A->matrix->n,
      C->matrix->m, C->matrix->n);
  fprintf(stderr, "{\n");
  ...
  while ( AL->matrix->m < A->matrix->m ){

	b = min(AR->matrix->m, BLOCK_SIZE);
...
//                            AB,            0, cj_TOP );
//
//  cj_Matrix_part_part_2x2( C,    CTL, CTR,
//                                 CBL, CBR,     0, 0, cj_TL );
//
//  while ( AT->matrix->m < A->matrix->m ){
//
